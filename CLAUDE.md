...
- [x] test-data-synthesiser implemented (2026-06-02): Node.js library + CLI that produces per-archetype demo CSVs. scripts/lib/synthesiser.mjs (PRNG, configs, weather fetch+cache, schedule, holidays, forward model, HW/cooking, elec baseload, noise injection, clamp, stats, CSV+JSON+MD outputs); scripts/synthesise.mjs CLI wrapper; 4 demo-configs (modern-out-for-work, average-in-all-day, small-and-efficient, big-old-draughty); test-data/noise-config.json vendored from praxis-hub; test-synthesiser.mjs TCs 1,2,3,5,6,10 pass offline; TC7 skips until real weather cache populated. Deviations D1-D4 in plan. Face-validity debug RESOLVED (2026-06-02): F8 HW+cooking cv decoupling + F9 SWR lower bound 0.95 applied (7943720, 294fa04); debug doc Phase 12 appended. Gas clamps ≤200 all archetypes. Continuous archetypes (average-in-all-day, big-old-draughty) pass all FV metrics. Twin-peak gas_hdd_r2 lifted to 0.604 / 0.649 (above 0.60 hard floor, below 0.65 target — structural calibration ceiling, accepted). SWR ≥0.95 all archetypes. Elec clamps 88–361 accepted as informational (F12c). Next: parameter iteration for annual-target gaps in Opus window.
- [x] m1-data-ingestion-v2 implemented (2026-06-03): full v2 delta — DST auto-detection (two-pass parseCSV, lastSundayOf/findTransitionDatesInRange/detectAutumnBack/detectSpringForward/detectCsvTimezone helpers, autumn-back UTC offset arithmetic, 9-case classification table); days_with_data gate (single <90 block in parseCSV + normaliseConsumption metadata field, replacing 7 scattered 30/90-day calendar checks); Octopus path timezone_detection stub + days_with_data block; CSV rate fields removed from index.html + app.js (interim Ofgem-cap defaults); gsp_region added to all 4 demo-configs; synthesiser extended to emit data/demos/{slug}.json (m1-contract format: timestamp/gas_kwh/elec_kwh per normaliseConsumption); 4 demo JSONs regenerated and committed; DEMO_ARCHETYPES + loadDemoArchetype exported from data-ingestion.js; runDemoPipeline + demo-triggers scaffold in app.js/index.html; stale CONFIG constants (MIN_DAYS_FOR_ANALYSIS, WARNING_DAYS_THRESHOLD, GAP_WARNING_PERCENTAGE) removed. Deviations D1-D3 in plan. Tests: M3 18/18, M5 39/39, M5b 29/29, M6 24/24, M7 39/39, M8 24/24, M9 24/24. Browser tests pending.
- [x] m3-baseload-v2 implemented (2026-06-04): full fork scope — 4 new BASELOAD_CONFIG constants (LOW_GAS_WARM_MAX_GAS_KWH_FRACTION/MIN_DAILY_MEAN_TEMP_C, WINTER_NON_HEAT_FRACTION, ELECTRICITY_BASELOAD_PERCENTILE); percentile() helper; applyLowGasWarmCorrection (F.5 exported); computeElectricityBaseload (I exported); applyElectricHeatingAttribution (J internal); detectSupplementaryLoads updated (userClassificationOverride param, skipped() closure, daily_gas_kwh in dailyData, 2% correction, corrected gate/confidence/3-tier classification, new return fields, electric_heating_is_primary removed); separateBaseload updated (3rd param, elec_heating_kwh/nonheat_residual_kwh in heating array, no-gas branch + gas branch wired for F.5/I/J); app.js: separateBaseload call updated to pass null, electric_heating_is_primary replaced with classification_effective checks. test-m3-v2.mjs: 19 test cases (44 assertions). Deviations D1-D3 in plan. Tests: M3-v2 44/44, M3-step-f 18/18, M5 39/39, M5b 29/29, M6 24/24, M7 39/39, M8 29/29, M9 24/24. Browser tests pending.
- [x] m4-heat-loss-v2 implemented (2026-06-04): full v2 rewrite — combined-fuel Siviour LHS (gas×η + elec×1.0); η-move (η leaves HTC recovery, enters LHS); presence-gated whole-day rule (gas_present/elec_present flags; absent fuel contributes 0 and does not gate; gap in present fuel excludes whole day); mintThermalHeatDelivered (per-HH series, always emitted including absence periods); applyHtcRescale named export (±20%-bounded setpoint rescale, dormant first pass: null payload → htc_used = htc); boiler efficiency 4-tier dropdown (INV-12, default 0.85); removal of no-gas short-circuit, Check 4D, floor_area/HLP, net_flow/internal_gains; solar_aperture_m2 renamed solar_aperture throughout; htc_low_plausibility source-blind flag; diagnostic retains htc_w_per_k and adds htc_used. index.html: boiler-efficiency → select, floor-area removed. app.js: 4-arg call site, removed floorAreaInput/floorAreaM2/htc_w_per_k_adjusted/hlp_w_per_m2_k refs, solar_aperture_m2 → solar_aperture in display + diagnostic. thermal-character.js + scenario-consumption.js: solar_aperture_m2 → solar_aperture (D1). test-m4-v2.mjs: 27/27 (T1–T26 + T21-int). Deviations D1-D2 in plan. Tests: M4-v2 27/27, M3-v2 44/44, M5 39/39, M5b 29/29, M6 24/24, M7 39/39, M8 29/29, M9 24/24. Browser tests pending.
- [x] m2-external-data-v2 implemented (2026-06-04): M2 becomes sole owner of operative base tariffs; wholesale stays M2-internal (removed from external[]). external-data.js: OFGEM_CAP_ELEC/GAS_BY_QUARTER constants (Q1 2024–Q4 2026, mostly PROVISIONAL); AGILE_REGIONAL_RATES 14-region D/P table; REGIONAL_STANDING_CHARGES (provisional national); quarterKey/getRateForTs/imputeWholesaleForSlot helpers; fetchAgileCalibration 3-path (calibrated/regional_table/national_default); buildBaseTariffs (exported — returns hh_rate/gas_rate/flat_rate/standing_charge/null_wholesale_fraction/coverage_warnings/non_null_wholesale_count); alignExternalData now weather-only (priceLookup removed); buildExternalMetadata updated (agile_calibration removed, coverage_warnings added); data/agile-regional-rates.csv + data/regional-standing-charges.csv created. pricing-engine.js: prepareRates refactored to (ingestion, m2Result, params) — reads pre-computed rate arrays from m2Result, applies §14 overrides; flat_rate_by_hh added to return; imputeWholesaleForSlot/isPeakHour/D_DEFAULT/P_DEFAULT removed; PE_CONFIG trimmed (SVT_RATE_DEFAULT_P/ELEC_STANDING_DEFAULT_P_DAY/GAS_STANDING_DEFAULT_P_DAY/HH_OVERHEAD_DEFAULT_P removed); electricityRateForHH uses flat_rate_by_hh[i] for dumb_hp_svt. app.js: buildBaseTariffs imported; readRateParams removes ofgem_cap_elec_p_kwh + null fallbacks; runExternalData wired to buildBaseTariffs, agile_calibration stored top-level; runScenarioConsumption/runPricingEngine pass externalResult as m2Result; displayPricingResults/buildEffectivePricingResult updated (national_default, unknown); populateDroveTile uses flat_rate_by_hh[0]; buildRateArrays removed. test-m2-v2.mjs created (47 assertions: TC10/13–19/SP→UTC). test-m8.mjs rewritten for m2Result fixture (29 assertions). package.json created (luxon devDependency). Deviations D1-D3 in plan. Tests: M3 18/18, M5 39/39, M5b 29/29, M6 24/24, M7 39/39, M8 29/29, M9 24/24, M2-v2 47/47. Browser tests pending.
