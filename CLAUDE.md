...
- [x] test-data-synthesiser implemented (2026-06-02): Node.js library + CLI that produces per-archetype demo CSVs. scripts/lib/synthesiser.mjs (PRNG, configs, weather fetch+cache, schedule, holidays, forward model, HW/cooking, elec baseload, noise injection, clamp, stats, CSV+JSON+MD outputs); scripts/synthesise.mjs CLI wrapper; 4 demo-configs (modern-out-for-work, average-in-all-day, small-and-efficient, big-old-draughty); test-data/noise-config.json vendored from praxis-hub; test-synthesiser.mjs TCs 1,2,3,5,6,10 pass offline; TC7 skips until real weather cache populated. Deviations D1-D4 in plan. Face-validity debug round 3 complete (2026-06-02): F6 signal-floor gate + F7 summer_winter lower bound applied (0c6bb64); debug doc Phase 10 appended. Gas clamps resolved (99–<88 across all archetypes). big-old-draughty and average-in-all-day pass all FV metrics. Returned to Opus for round 4 design decision: OI-6 twin-peak gas_hdd_r2 structurally below 0.60 (model calibration, not a bug), OI-7 small-and-efficient summer_winter_ratio 0.972 below 1.05 (F7 insufficient), OI-8 elec clamps 157–301 from event-spike AR(1) leak. Head: see latest commit.
